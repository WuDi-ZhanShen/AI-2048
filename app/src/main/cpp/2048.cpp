...
float score_toplevel_move(board_t board, int move) {
    ...
    eval_state state;
    state.depth_limit = std::max(3, maxDepth == 0 ? count_distinct_tiles(board) - 2 : std::min(maxDepth, count_distinct_tiles(board) - 2));
...
Java_com_game2048_MainGame_nativeGetBestMove(JNIEnv *env, jclass clazz, jobjectArray grid) {
    board_t board = 0;
...
Java_com_game2048_MainGame_nativeSetMaxDepth(JNIEnv *env, jclass clazz, jint depth) {
    maxDepth = depth;
}