...
public class GameState {
    ...
    public GameState(int[][] grid) {
        cellMatrix = new int[grid.length][grid[0].length];
     ...
     */
    public int evaluate(int smoothWeight, int monoWeight, int emptyWeight, int maxWeight) {
        return smoothness() * smoothWeight
                + monotonicity() * monoWeight
                + getEmptyNum() * emptyWeight
                + maxValue() * maxWeight;
    }

    public int getEmptyNum() {
        int count = 0;
        for (int[] row : cellMatrix) {
            for (int cell : row) {
                if (cell == 0) {
                    count++;
                }
                    ...
                    for (int direction = 1; direction <= 2; direction++) {
                        ...
                        if (isInBounds(cnt_x, cnt_y)) {
                            if (cellMatrix[cnt_x][cnt_y] != 0) {
                                int targetValue = log2(cellMatrix[cnt_x][cnt_y]);
    ...
    public int monotonicity() {
        ...
        for (int[] matrix : cellMatrix) {
            ...
            while (next < cellMatrix[0].length) {
                ...
                int currentValue = log2(matrix[current]);
                int nextValue = log2(matrix[next]);
        ...
        for (int y = 0; y < cellMatrix[0].length; y++) {
            ...
            while (next < cellMatrix.length) {
                while (next < cellMatrix.length && cellMatrix[next][y] == 0) next++;
                if (next >= cellMatrix.length) next--;
                int currentValue = log2(cellMatrix[current][y]);
                int nextValue = log2(cellMatrix[next][y]);
        ...
        if (action == 3 || action == 0) {
            ...
            for (int i = 0; i < mergeRow.length - 1; i++) {
                ...
                for (int j = i + 1; j < mergeRow.length; j++) {
                    ...
                    if (mergeRow[i] != mergeRow[j]) break;
    ...
    public int islands() {
        ...
        for (int x = 0; x < cellMatrix.length; x++) {
            for (int y = 0; y < cellMatrix[0].length; y++) {
                if (cellMatrix[x][y] != 0 && !marked[x][y]) {
                    islands++;
                    mark(x, y, cellMatrix[x][y]);
                }
            }
        }
    ...
    private void mark(int x, int y, int value) {
        if (x >= 0 && x <= cellMatrix.length - 1 && y >= 0 && y <= cellMatrix[0].length - 1 && (cellMatrix[x][y] != 0)
                && (cellMatrix[x][y] == value) && (!marked[x][y])) {
            marked[x][y] = true;
